...
const AddyForm = ({ checkoutToken }) => {
  ...
  const objCountries = Object.entries(countries).map(([code, name]) => ({ id: code, label: name }))
  const objProvinces = Object.entries(provinces).map(([code, name]) => ({ id: code, label: name }))
  const objShipping = options.map((sO) => ({ id: sO.id, label: `${sO.description} - (${sO.price.formatted_with_symbol})` }))


  const getCountries = async (addyToken) => {
    const { countries } = await commerce.services.localeListShippingCountries(addyToken)
  ...
  const getOptions = async (addyToken, country, stateProvince=null) => {
    const options = await commerce.checkout.getShippingOptions(addyToken, {country, region: stateProvince})
    setOptions(options)
  ...
  useEffect(() => {
    ...
    getOptions(checkoutToken.id, country, province)
  }, [country, province])
        ...
        <form>
          <Grid container spacing={3} sx={{ justifyContent: "center" }}>
            ...
            <Grid item xs={12} sm={6}>
              ...
              <Select value={province} fullWidth onChange={(e) => setProvince(e.target.value)}>
